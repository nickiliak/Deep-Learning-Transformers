...
def calculate_bits_per_character(model, tokenizer, val_loader, device):
    ...
    # We need original text to count characters
    # This is an approximation - in practice you'd store original texts
...
def main():
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            save_path = os.path.join(repo_root, "models", "lstm_bpe_best.pt")
            torch.save({
                'epoch': epoch,
    ...
    # Save final model
    final_path = os.path.join(repo_root, "models", "lstm_bpe_final.pt")
    torch.save({
        'model_state_dict': model.state_dict(),
        'vocab_size': vocab_size,
        'test_loss': test_loss,
